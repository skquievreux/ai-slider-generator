...
export function PresentationForm() {
  const { formData, setFormData, setIsGenerating, setGeneratedSlides } = usePresentationStore()
  ...
  const handleInputChange = (field: keyof PresentationFormData, value: string | number) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            >
              {Array.from({ length: 21 }, (_, i) => i + 5).map(num => (
                <option key={num} value={num}>{num} Folien</option>
              ))}
            </select>
          </div>
        ...
        >
          {isSubmitting ? 'Generiere Präsentation...' : 'Präsentation generieren'}
        </button>
      </form>
    </div>
  )
