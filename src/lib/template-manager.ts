...
export class TemplateManager {
  ...
  static updateTemplateGoogleSlidesId(templateId: string, googleSlidesId: string): void {
    const template = this.getTemplate(templateId)
    if (template) {
      (template as any).googleSlidesTemplateId = googleSlidesId
      this.saveTemplates()
      console.log(`Updated template ${templateId} with Google Slides ID: ${googleSlidesId}`)
    }
  }
  ...
  static validateTemplate(template: TemplateConfig): { valid: boolean; errors: string[] } {
    const errors: string[] = []
