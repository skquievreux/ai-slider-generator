  ...
  try {
    ...
    const textElements = pageElements.filter(element => {
      ...
      return shape?.text?.textElements &&
             shape?.text?.textElements.length > 0
    })

    console.log(`Found ${textElements.length} text elements:`, textElements.map(el => ({
      id: el.objectId,
    ...
    if (textElements.length === 0) {
      ...
      const anyTextElements = pageElements.filter(element => {
        ...
        return textElements && textElements.some(te =>
          te.textRun?.content && te.textRun.content.trim().length > 0
        )
      })

      console.log(`Fallback: Found ${anyTextElements.length} elements with text content`)
      ...
      if (anyTextElements.length > 0) {
        ...
        console.error('No text elements found in template. Page elements:', pageElements.map(el => ({
          id: el.objectId,
          type: el.shape?.shapeType || 'unknown',
          hasText: !!el.shape?.text
        })))
        throw new Error(`Template slide has no text elements. Please ensure the template has at least one text box or shape with text. Found ${pageElements.length} elements.`)
      }
    } else {
      textElementId = textElements[0].objectId!
    }

    console.log(`Using text element: ${textElementId} (type: ${textElements[0]?.shape?.shapeType || 'created'})`)
    ...
    body.slides.forEach((slide, index) => {
      ...
      if (slide.content.body && Array.isArray(slide.content.body) && slide.content.body.length > 0) {
